...
def _deploy_math(platon_tester):
    ...
    assert math_address
    math_code = platon_tester.get_code(math_address)
    assert len(math_code) > 2
    return math_address


def _make_call_math_transaction(platon_tester, contract_address, fn_name, fn_args=None):
