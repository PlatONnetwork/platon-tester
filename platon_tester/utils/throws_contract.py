...
def _deploy_throws(platon_tester, contract_name):
    deploy_hash = platon_tester.send_transaction({
        "from": platon_tester.get_accounts()[0],
        "gas": 500000,
        "data": THROWS_BYTECODE[contract_name],
    })
    deploy_receipt = platon_tester.get_transaction_receipt(deploy_hash)
    throws_address = deploy_receipt['contract_address']
    assert throws_address
    throws_code = platon_tester.get_code(throws_address)
    assert len(throws_code) > 2
    return throws_address


def _make_call_throws_transaction(platon_tester, contract_address, contract_name,
                                  fn_name, fn_args=None):
    from platon_abi import encode_abi
...
def _decode_throws_result(contract_name, fn_name, result):
    from platon_abi import decode_abi
